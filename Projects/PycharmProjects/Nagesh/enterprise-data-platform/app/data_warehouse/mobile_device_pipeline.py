...
from app.data_warehouse.pipeline import DataWareHousePipeline
...
class MobileDevice(DataWareHousePipeline):
    ...
    def load_data_into_target(self):
        ...
        for i in chunk_ranges(len(rows_list), self.row_commit_frequency):
            if not self.use_multi_processes:
                ...
                job = mp.Process(target=upsert_data_static,
                                 args=(schema, table, target_cols, values, rows_list[i[0]:i[1]],
                                       self.config.get(TARGET, DB_NAME)))
                self.processes.append(job)
                job.start()

                exec_log.info(
                    "started process with pid {0} for the range {1}-{2}".format(str(job.pid()), str(i[0]), str(i[1])))
    ...
    def upsert_data(self, schema, table, target_cols, values, data, new_correction=False):
        ...
        try:
            ...
            insert_df_into_table(self.tgt_connection if new_correction else OBDConnection(
                self.config.get(TARGET, DB_NAME)).connect_to_db(), schema, table, target_cols, values, data)
...
def upsert_data_static(schema, table, target_cols, values, data, DB_NAME):
    exec_log.info("inserting data into table")
    try:
        conn = OBDConnection(DB_NAME).connect_to_db()
        insert_df_into_table(conn,
                             schema, table, target_cols, values, data)
